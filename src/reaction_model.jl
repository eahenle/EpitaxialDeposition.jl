...
@memoize K(T::Float64)::Float64   = π * (PARAMS[:reactor][:wafer_diameter] / 2)^2 * PARAMS[:rate][:K0]  * exp(-PARAMS[:energy][:E]   / (PARAMS[:R][:kcal_mol_K] * T)) # [cm/s]
@memoize K2(T::Float64)::Float64  = π * (PARAMS[:reactor][:wafer_diameter] / 2)^2 * PARAMS[:rate][:K20] * exp(-PARAMS[:energy][:Ea2] / (PARAMS[:R][:kcal_mol_K] * T)) # [cm/s]
@memoize K3(T::Float64)::Float64  = π * (PARAMS[:reactor][:wafer_diameter] / 2)^2 * PARAMS[:rate][:K30] * exp(-PARAMS[:energy][:Ea3] / (PARAMS[:R][:kcal_mol_K] * T)) # [cm/s]
@memoize kKp(T::Float64)::Float64 = π * (PARAMS[:reactor][:wafer_diameter] / 2)^2 * PARAMS[:kKp][:k] * 10^(PARAMS[:kKp][:Ea] - PARAMS[:kKp][:b] / T) # [∅]
